...
var config = {
  ...
  "render": function (e) {
    ...
    config.elements.excludelist.value = e.excludelist.join(", ");
    config.elements.includelist.value = e.includelist.join(", ");
    ...
    /*  */
    for (let id in e.rules) config.listener.add(e.rules[id]);
  },
  "load": function () {
    ...
    config.elements.modal = document.getElementById("modal");
    ...
    config.elements.preflight = document.getElementById("preflight");
    ...
    config.elements.excludelist = document.getElementById("excludelist");
    config.elements.includelist = document.getElementById("includelist");
    ...
    config.elements.excludelist.addEventListener("change", config.listener.excludelist);
    config.elements.includelist.addEventListener("change", config.listener.includelist);
    ...
    config.elements.modal.querySelector(".close").addEventListener("click", config.listener.modal.hide);
    config.elements.preflight.addEventListener("click", function () {
      config.listener.modal.show("url('images/preflight.png')");
    });
    /*  */
  ...
  "listener": {
    ...
    "domain": function () {
      const inputs = [...document.querySelectorAll("input[name='domain']")];
      const domainType = inputs.filter(e => e.checked)[0].id;
      ...
      if (e) {
        if (e.target) {
          if (e.target === config.elements.modal) {
            config.listener.modal.hide();
          }
        }
      }
    },
    "methods": function (e) {
      let value = "GET, PUT, POST, DELETE, HEAD, OPTIONS";
      if (e.target.value) {
    ...
    "modal": {
      "hide": function () {
        if (config.elements.modal) {
          config.elements.modal.style.display = "none";
          config.elements.modal.style.backgroundImage = "unset";
        }
      },
      "show": function (e) {
        if (config.elements.modal) {
          config.elements.modal.style.display = "block";
          config.elements.modal.style.backgroundImage = e;
        }
      }
    },
    "origin": {
    ...
    "excludelist": function (e) {
      ...
      /*  */
      background.send("excludelist", domains);
    },
    "includelist": function (e) {
      let domains = [];
      if (e) {
        if (e.target) {
          if (e.target.value) {
      ...
      /*  */
      background.send("includelist", domains);
    },
    "add": function (rule) {
      ...
      if (node) {
        ...
        if (table) {
          ...
          const order = config.elements.custom.children.length + 1;
          /*  */
          ...
          /*  */
          if (rule) {
            const current = rule.order ? rule.order : order;
            /*  */
            table.style.order = current;
            table.setAttribute("id", rule.id);
            table.setAttribute("order", current);
            ...
            table.querySelector(".number").value = "Rule " + current;
            ...
            const max = 1000000;
            /*  */
            table.style.order = order;
            table.setAttribute("order", order);
            table.setAttribute("state", "temporary");
            table.querySelector(".state").checked = true;
            table.querySelector(".number").value = "Rule " + order;
            table.setAttribute("id", Math.floor(Math.random() * (max - min) + min));
          }
        }
        /*  */
        config.elements.custom.appendChild(node);
        if (!rule) container.scrollTo({"behavior": "smooth", "top": container.scrollHeight});
      }
    },
    "save": function (e) {
      if (e) {
